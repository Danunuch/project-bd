...
if (isset($_GET['service_id']) && isset($_GET['lang'])) {
	...
	if ($lang == "en") {
		$service_detail = $conn->prepare("SELECT * FROM service_en WHERE id = :id");
		$service_detail->bindParam(":id", $service);
		$service_detail->execute();
		$detail_service = $service_detail->fetch(PDO::FETCH_ASSOC);
	} else {
		$service_detail = $conn->prepare("SELECT * FROM service_th WHERE id = :id");
		$service_detail->bindParam(":id", $service);
		$service_detail->execute();
		$detail_service = $service_detail->fetch(PDO::FETCH_ASSOC);
	}
	...
	<main>
		...
		<header>
			...
			<nav hidden>
				...
				<ul class="attributes">
					<li><a href="?service_id=<?php echo $service_id?>&lang=th" <?php
											if (!isset($_GET['lang'])) {
												echo "class='active'";
					...
					<li><a href="?service_id=<?php echo $service_id?>&lang=en" <?php
											if (!isset($_GET['lang'])) {
												echo "class='not_active'";
											} else if (isset($_GET['lang'])) {
												$lang = $_GET['lang'];
		...
		<section id="page-section">
			<div class="container-xxl">
				...
				<h4><?= $detail_service["title_service"] ?></h4>

				<span class="text-dark mb-4 d-inline-block">
					<?php
					$dateTime = $detail_service["created_service"];
					$splitTime = explode(" ", $dateTime);
					$date = $splitTime[0];
					echo $date;
					?>
				</span>
				<p><?= $detail_service["paragraph1"] ?></p>
				<p><?= $detail_service["paragraph2"] ?></p>
				<p><?= $detail_service["paragraph3"] ?></p>
				<p><?= $detail_service["paragraph4"] ?></p>
				<div class="row mt-4">
					<?php
					//ตัดนามสกุลไฟล์
					$row_img1 = explode(".", $detail_service["service_img1"]);
					$row_img2 = explode(".", $detail_service["service_img2"]);
					$row_img3 = explode(".", $detail_service["service_img3"]);
					?>
					<div class="col-6 col-md-4">
						<div class="view-seventh mb-4">
							<a href="webpanelcw/assets/service_upload/<?= $detail_service["service_img1"] ?>" class="b-link-stripe b-animate-go thickbox" title="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
								<div class="box-gallery">
									<div class="bg-img <?php if ($row_img1[1] == null) {
															echo 'img-none';
														} ?>">
										<img class="img-fluid" src="webpanelcw/assets/service_upload/<?= $detail_service["service_img1"] ?>" alt="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
									</div>
					...
					<div class="col-6 col-md-4">
						<div class="view-seventh mb-4">
							<a href="webpanelcw/assets/service_upload/<?= $detail_service["service_img2"] ?>" class="b-link-stripe b-animate-go thickbox" title="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
								<div class="box-gallery">
									<div class="bg-img <?php if ($row_img2[1] == null) {
															echo 'img-none';
														} ?>">
										<img class="img-fluid" src="webpanelcw/assets/service_upload/<?= $detail_service["service_img2"] ?>" alt="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
									</div>
					...
					<div class="col-6 col-md-4">
						<div class="view-seventh mb-4">
							<a href="webpanelcw/assets/service_upload/<?= $detail_service["service_img3"] ?>" class="b-link-stripe b-animate-go thickbox" title="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
								<div class="box-gallery">
									<div class="bg-img <?php if ($row_img3[1] == null) {
															echo 'img-none';
														} ?>">
										<img class="img-fluid" src="webpanelcw/assets/service_upload/<?= $detail_service["service_img3"] ?>" alt="7 วัสดุปูพื้นเลือกยังไงให้เหมาะกับบ้านคุณ">
									</div>
