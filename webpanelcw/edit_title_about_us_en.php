    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img1['size'] > 0 && $img1['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                $edit = $conn->prepare("UPDATE about_us_en SET title = :title, img = :img WHERE id = :id");
                ...
                if ($edit) {
                    ...
                    echo "<meta http-equiv='Refresh' content='0.001; url=about_us_en'>";
                } else {
        ...
        $edit = $conn->prepare("UPDATE about_us_en SET title = :title WHERE id = :id");
        $edit->bindParam(":title", $title);
        $edit->bindParam(":id", $topic_id);
        $edit->execute();
