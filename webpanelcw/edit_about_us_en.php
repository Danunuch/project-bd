    ...
    if (in_array($fileActExt1, $allow)) {
        if ($img1['size'] > 0 && $img1['error'] == 0) {
            if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                $edit = $conn->prepare("UPDATE about_us_en SET content = :content, img = :img");
                ...
                if ($edit) {
                    ...
                    echo "<meta http-equiv='Refresh' content='0.001; url=about_us?lang=en'>";
                } else {
        ...
        $edit = $conn->prepare("UPDATE about_us_en SET content = :content");
        $edit->bindParam(":content", $content);
        $edit->execute();
        if ($edit) {
            echo '<script>alert("แก้ไขข้อมูลสำเร็จ")</script>';
...
<div class="layout-container" style="position: absolute; top: 80px; z-index: 1;">
    ...
    <div class="layout-pages">
        <div class="box-title">
            <p class="add-blog">แก้ไขเกี่ยวกับเรา (ภาษาอังกฤษ)</p>
        </div>
        <?php
        if (isset($errorMsg)) { ?>
