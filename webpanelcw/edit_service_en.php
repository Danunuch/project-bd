...
if (isset($_GET['service_id'])) {
    ...
    $data_service = $conn->prepare("SELECT * FROM service_en WHERE id = :id");
    $data_service->bindParam(":id", $service_id);
    $data_service->execute();
    $row_data_service = $data_service->fetch(PDO::FETCH_ASSOC);
}
            ...
            if (in_array($fileActExt1, $allow) && in_array($fileActExt2, $allow) && in_array($fileActExt3, $allow)) {
                if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0 && $img3['size'] > 0 && $img3['error'] == 0) {
                    if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2) && move_uploaded_file($img3['tmp_name'], $filePath3)) {
                        $insert_service = $conn->prepare("UPDATE service_en SET title_service = :title_service, paragraph1 = :paragraph1,
                                                 paragraph2 = :paragraph2, paragraph3 = :paragraph3, paragraph4 = :paragraph4,
                                                  service_img1 = :service_img1, service_img2 = :service_img2, service_img3 = :service_img3 WHERE id = :id");
                        ...
                        $insert_service->bindParam(":id", $service_id);
                        $insert_service->execute();
                        if ($insert_service) {
                            echo "<script>alert('แก้ไขบริการเรียบร้อยแล้ว')</script>";
                ...
                if ($img1['size'] > 0 && $img1['error'] == 0 && $img2['size'] > 0 && $img2['error'] == 0) {
                    if (move_uploaded_file($img1['tmp_name'], $filePath1) && move_uploaded_file($img2['tmp_name'], $filePath2)) {
                        $insert_service = $conn->prepare("UPDATE service_en SET title_service = :title_service, paragraph1 = :paragraph1,
                                                 paragraph2 = :paragraph2, paragraph3 = :paragraph3, paragraph4 = :paragraph4,
                                                  service_img1 = :service_img1, service_img2 = :service_img2 WHERE id = :id");
                ...
                if ($img1['size'] > 0 && $img1['error'] == 0) {
                    if (move_uploaded_file($img1['tmp_name'], $filePath1)) {
                        $insert_service = $conn->prepare("UPDATE service_en SET title_service = :title_service, paragraph1 = :paragraph1,
                                                 paragraph2 = :paragraph2, paragraph3 = :paragraph3, paragraph4 = :paragraph4,
                                                  service_img1 = :service_img1 WHERE id = :id");
                ...
                $insert_service->bindParam(":paragraph3", $paragraph3);
                $insert_service->bindParam(":paragraph4", $paragraph4);
                $insert_service->bindParam(":service_img1", $row_data_service['service_img1']);
                $insert_service->bindParam(":service_img2", $row_data_service['service_img2']);
                $insert_service->bindParam(":service_img3", $row_data_service['service_img3']);
                $insert_service->bindParam(":id", $service_id);
                $insert_service->execute();
                if ($insert_service) {
                    echo "<script>alert('แก้ไขบริการเรียบร้อยแล้ว')</script>";
...
<div class="layout-container" style="position: absolute; top: 80px; z-index: 1;">
    ...
    <div class="layout-pages">
        <div class="box-title">
            <p class="add-service">เเก้ไขบริการ (ภาษาไทย)</p>
        </div>
        <?php
        if (isset($errorMsg)) { ?>
        ...
        <div class="box-form">
            <form method="post" class="form-lo" enctype="multipart/form-data">
                ...
                <input type="text" name="title_service" value="<?= $row_data_service["title_service"] ?>" id="title_service" class="form-control ">
                <div class="pt">
                    <div class="box-txt-content2">
                        <label for="paragraph1" class="mt-3 paragraph1">เนื้อหาย่อหน้าที่ 1 (จำเป็น)</label>
                        <textarea type="text" name="paragraph1" id="paragraph1" class="form-control "><?= $row_data_service["paragraph1"] ?></textarea>

                        <label for="paragraph3" class="mt-3 paragraph3">เนื้อหาย่อหน้าที่ 3 (ไม่จำเป็น)</label>
                        <textarea type="text" name="paragraph3" id="paragraph3" class="form-control "><?= $row_data_service["paragraph2"] ?></textarea>
                    ...
                    <div class="box-txt-content1">
                        ...
                        <textarea type="text" name="paragraph2" id="paragraph2" class="form-control "><?= $row_data_service["paragraph3"] ?></textarea>
                        <label for="paragraph4" class="mt-3 paragraph4">เนื้อหาย่อหน้าที่ 4 (ไม่จำเป็น)</label>
                        <textarea type="text" name="paragraph4" id="paragraph4" class="form-control "><?= $row_data_service["paragraph4"] ?></textarea>

                        <div class="txt-img mt-4"><span>รูปภาพเดิม</span></div>
                        <div class="pos">
                            <?php
                            $row_img1 = explode(".", $row_data_service["service_img1"]);
                            $row_img2 = explode(".", $row_data_service["service_img2"]);
                            $row_img3 = explode(".", $row_data_service["service_img3"]);
                            ?>
                            <div class="filewrap">
                                <img width="100%" src="assets/service_upload/<?php if ($row_data_service["service_img1"] == null) {
                                                                                echo 'file-upload.png';
                                                                            } else {
                                                                                echo $row_data_service['service_img1'];
                                                                            } ?>" alt="ไม่มีรูปภาพ">
                            </div>
                            <div class="filewrap">
                                <img width="100%" src="assets/service_upload/<?php if ($row_data_service["service_img2"] == null) {
                                                                                echo 'file-upload.png';
                                                                            } else {
                                                                                echo $row_data_service['service_img2'];
                                                                            } ?>" alt="ไม่มีรูปภาพ">
                            </div>
                            <div class="filewrap">
                                <img width="100%" src="assets/service_upload/<?php if ($row_data_service["service_img3"]== null) {
                                                                                echo 'file-upload.png';
                                                                            } else {
                                                                                echo $row_data_service['service_img3'];
                                                                            } ?>" alt="ไม่มีรูปภาพ">
                            </div>
                        </div>
                    </div>
                ...
                <div class="box-btn "> <button type="submit" name="edit-service" class="btn-service-submit">แก้ไขข้อมูล</button></div>
            </form>
        </div>
    </div>
</div>
